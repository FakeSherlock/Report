...
@app.route('/index')
def index():
    return render_template("upload.html", all_list=sorted(Photo.all(), key=lambda list: list.date, reverse=True))
...
def upload_db():
    ...
    if post_data and allowed_file(post_data.filename):
        ...
        upload_data.memo = request.form['memo']
        upload_data.put()
    ...
    return render_template("upload.html", comment=comment, all_list=sorted(Photo.all(), key=lambda list: list.date, reverse=True))
